...
let texto = `*Idioma de AzkanBot cambiado Correctamente:* `
let texto2 = `*Seleccione el idioma para AzkanBot*`
...
let idioma2 = await translate(`${texto2}`, { to: bahasaAK.lenguaje(), autoCorrect: true })
let idioma3 = await translate(`${texto3}`, { to: bahasaAK.lenguaje(), autoCorrect: true })

try {  
if (args[0] == 'es'){
global.bahasaAK = es
await conn.sendButton(m.chat, bahasaAK['smsAvisoEG']() + idioma.text + '\n' + 'დ ```Español```', wm, null, [[`☘️ 𝗠 𝗘 𝗡 𝗨`, `${usedPrefix}menu`]], fkontak, m)
...
handler.command = /^(idioma|languaje|idiomas|languajes|languages|bahasa)$/i
handler.owner = true
