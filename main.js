...
global.authFile = `AzkanBotSession`
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['Azkan coy', 'Chrome', '2.0.0'] : methodCodeQR ? ['Azkan coy', 'Chrome', '2.0.0'] : ['Chrome (Linux)', '', ''],
auth: {
creds: state.creds,
...
console.log(chalk.bold.yellow(bahasaAK['smsCodigoQR']()))}
}
if (connection == 'open') {
console.log(chalk.bold.greenBright(bahasaAK['smsConexion']()))}
let reason = new Boom(lastDisconnect?.error)?.output?.statusCode
if (reason == 405) {
await fs.unlinkSync("./AzkanBotSession/" + "creds.json")
...
console.log(chalk.bold.cyanBright(bahasaAK['smsConexionOFF']()))
} else if (reason === DisconnectReason.connectionClosed) {
console.log(chalk.bold.magentaBright(bahasaAK['smsConexioncerrar']()))
await global.reloadHandler(true).catch(console.error)
} else if (reason === DisconnectReason.connectionLost) {
console.log(chalk.bold.blueBright(bahasaAK['smsConexionperdida']()))
await global.reloadHandler(true).catch(console.error)
} else if (reason === DisconnectReason.connectionReplaced) {
console.log(chalk.bold.yellowBright(bahasaAK['smsConexionreem']()))
} else if (reason === DisconnectReason.loggedOut) {
console.log(chalk.bold.redBright(bahasaAK['smsConexionOFF']()))
await global.reloadHandler(true).catch(console.error)
} else if (reason === DisconnectReason.restartRequired) {
console.log(chalk.bold.cyanBright(bahasaAK['smsConexionreinicio']()))
await global.reloadHandler(true).catch(console.error)
} else if (reason === DisconnectReason.timedOut) {
console.log(chalk.bold.yellowBright(bahasaAK['smsConexiontiem']()))
await global.reloadHandler(true).catch(console.error) //process.send('reset')
} else {
console.log(chalk.bold.redBright(bahasaAK['smsConexiondescon'](reason, connection)))
}}
}
...
conn.welcome = bahasaAK['smsWelcome']() 
conn.bye = bahasaAK['smsBye']() 
conn.spromote = bahasaAK['smsSpromote']() 
conn.sdemote = bahasaAK['smsSdemote']() 
conn.sDesc = bahasaAK['smsSdesc']() 
conn.sSubject = bahasaAK['smsSsubject']() 
conn.sIcon = bahasaAK['smsSicon']() 
conn.sRevoke = bahasaAK['smsSrevoke']() 
...
} else conn.logger.info(`MENDETEKSI PLUGIN ♻️ : '${filename}'`)
const err = syntaxerror(readFileSync(dir), filename, {
sourceType: 'module',
allowAwaitOutsideFunction: true,
});
if (err) conn.logger.error(`RUSAK MEMUAT ULANG | SYNTAX ERROR WHILE LOADING '${filename}'\n${format(err)}`);
else {
try {
...
let prekey = []
let directorio = readdirSync("./AzkanBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./AzkanBotSession/${files}`)
})
} 
...
console.log(chalk.bold.green(bahasaAK.smspurgeSessionSB1()))
} else {
console.log(chalk.bold.cyanBright(bahasaAK.smspurgeSessionSB2()))
}} catch (err) {
console.log(chalk.bold.red(bahasaAK.smspurgeSessionSB3() + err))
}}

function purgeOldFiles() {
const directories = ['./AzkanBotSession/', './GataJadiBot/']
...
if (err) {
console.log(chalk.bold.red(`${bahasaAK.smspurgeOldFiles3()} ${file} ${bahasaAK.smspurgeOldFiles4()}` + err))
} else {
console.log(chalk.bold.green(`${bahasaAK.smspurgeOldFiles1()} ${file} ${bahasaAK.smspurgeOldFiles2()}`))
} }) }
}) }) }) }

setInterval(async () => {
await clearTmp()
console.log(chalk.bold.cyanBright(bahasaAK.smsClearTmp()))}, 1000 * 60 * 4) // 4 min 

setInterval(async () => {
await purgeSession()
console.log(chalk.bold.cyanBright(bahasaAK.smspurgeSession()))}, 1000 * 60 * 10) // 10 min
...
setInterval(async () => {
await purgeOldFiles()
console.log(chalk.bold.cyanBright(bahasaAK.smspurgeOldFiles()))}, 1000 * 60 * 10)

_quickTest().then(() => conn.logger.info(chalk.bold(bahasaAK['smsCargando']().trim()))).catch(console.error)

let file = fileURLToPath(import.meta.url)
watchFile(file, () => {
unwatchFile(file)
console.log(chalk.bold.greenBright(bahasaAK['smsMainBot']().trim()))
