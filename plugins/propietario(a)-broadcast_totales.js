...
await conn.reply(m.chat, bahasaAK.smsBCMensaje2(), m)
...
setTimeout(async () => {
await conn.reply(group, `${bahasaAK.smsBCbot7()}\n\n` + teks4, { mentions: usersTag2 }, { quoted: fkontak });
}, delay)}
for (let user of chats2) {
await new Promise(resolve => setTimeout(resolve, 2000)) // 2 segundos
await conn.reply(user, `${bahasaAK.smsBCbot7()}\n\n` + teks4, fkontak, null)
totalPri2++
if (totalPri2 >= 500000) { 
break
}}  
...
await m.reply(`${bahasaAK.smsBCbot1()}
\`\`\`${bahasaAK.smsBCbot2()} >> ${totalPrivate2}\`\`\`
\`\`\`${bahasaAK.smsBCbot3()} >>   ${totalGroups2}\`\`\`
\`\`\`${bahasaAK.smsBCbot4()} >>   ${total2}\`\`\`\n\n*${bahasaAK.smsBCbot5()} ${time2}*\n${totalPri2 >= 500000 ? `\n*${bahasaAK.smsBCbot6()}*` : ''}`)        
}
