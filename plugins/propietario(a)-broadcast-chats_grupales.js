...
if (!text && !m.quoted) return m.reply(bahasaAK.smsBCMensaje(usedPrefix, command))        
...
let locale = bahasaAK.lenguaje()
let dia = d.toLocaleDateString(locale, { weekday: 'long' })
let fecha = d.toLocaleDateString(bahasaAK.lenguaje(), { day: 'numeric', month: 'numeric', year: 'numeric' })
let mes = d.toLocaleDateString(bahasaAK.lenguaje(), { month: 'long' })
let año = d.toLocaleDateString(bahasaAK.lenguaje(), { year: 'numeric' })
...
await m.reply(bahasaAK.smsChatGP1())   
for (let i = 0; i < groups.length; i++) {
const id = groups[i];
const infoGP = bahasaAK.smsChatGP2(readMS, dia, mes, año, fecha, tiempo)
...
await m.reply(bahasaAK.smsChatGP3(totalGP))
}     
