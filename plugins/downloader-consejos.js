...
let result = await translate(`${contenido}`, { to: bahasaAK.lenguaje(), autoCorrect: true })
let texto = `
・☘️・》・》・》
...
//await conn.sendButton(m.chat, texto.trim(), wm, img, [[bahasaAK.smsConj(), `${usedPrefix + command}`], [bahasaAK.smsConMenu(), `${usedPrefix}menu`]], m, frep)}   
...
let list = (await axios.get(`https://raw.githubusercontent.com/GataNina-Li/AzkanBot-MD/master/src/JSON/frase2.json`)).data  
let contenido = await list[Math.floor(list.length * Math.random())]
let frase = contenido.motivasi
let frase2 = await translate(`${frase}`, { to: bahasaAK.lenguaje(), autoCorrect: true })
let texto = `
・☘️・》・》・》
...
//await conn.sendButton(m.chat, texto.trim(), wm, img, [[bahasaAK.smsFras(), `${usedPrefix + command}`], [bahasaAK.smsConMenu(), `${usedPrefix}menu`]], m, frep)} 
} catch (e) {
...
handler.command = ['motifasi', 'cape', 'frase2', 'phrase2']
